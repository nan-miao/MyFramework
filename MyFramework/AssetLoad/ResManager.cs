...
namespace MyFramework.AssetLoad
{
    /// <summary>
    ...
    public class ResManager : BaseManager<ResManager>
    {
        ...
        {
            var resName = path + "_" + typeof(T).Name;
            ResInfo<T> info;
            //字典中不存在资源时
        ...
        private IEnumerator ReallyLoadAsync<T>(string path) where T : Object
        {
            ...
            if (resDic.ContainsKey(resName))
            {
                var resInfo = resDic[resName] as ResInfo<T>;
                //取出资源信息 并且记录加载完成的资源
        ...
        private IEnumerator ReallyLoadAsync(string path, Type type)
        {
            ...
            if (resDic.ContainsKey(resName))
            {
                var resInfo = resDic[resName] as ResInfo<Object>;
                //取出资源信息 并且记录加载完成的资源
        ...
        public void UnloadAsset(string path, Type type, bool isDel = false, UnityAction<Object> callBack = null,
            bool isSub = true)
        {
            var resName = path + "_" + type.Name;
            //判断是否存在对应资源
        ...
        private IEnumerator ReallyUnloadUnusedAssets(UnityAction callBack)
        {
            ...
            foreach (var path in resDic.Keys)
                if (resDic[path].refCount == 0)
                    list.Add(path);
            foreach (var path in list) resDic.Remove(path);
        ...
        {
            var resName = path + "_" + typeof(T).Name;
            if (resDic.ContainsKey(resName)) return (resDic[resName] as ResInfo<T>).refCount;
            return 0;
        }
