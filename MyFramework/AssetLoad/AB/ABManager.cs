...
namespace MyFramework.AssetLoad.AB
{
//知识点
//字典
//协程
...
//单例模式基类——>观看Unity小框架视频 进行学习
    public class ABManager : SingletonAutoMono<ABManager>
    {
        ...
        private void LoadDependencies(string abName)
        {
            ...
            LoadMainAB();
            //获取依赖包
            var strs = manifest.GetAllDependencies(abName);
            for (var i = 0; i < strs.Length; i++)
                if (!abDic.ContainsKey(strs[i]))
                {
                    var ab = AssetBundle.LoadFromFile(PathUrl + strs[i]);
                    abDic.Add(strs[i], ab);
                }
        }
        ...
        public T LoadRes<T>(string abName, string resName) where T : Object
        {
            ...
            {
                var ab = AssetBundle.LoadFromFile(PathUrl + abName);
                abDic.Add(abName, ab);
            }

            //得到加载出来的资源
            var obj = abDic[abName].LoadAsset<T>(resName);
        ...
        public void LoadResAsync<T>(string abName, string resName, UnityAction<T> callBack, bool isSync = false)
            where T : Object
        {
        ...
        //正儿八经的 协程函数
        private IEnumerator ReallyLoadResAsync<T>(string abName, string resName, UnityAction<T> callBack, bool isSync)
            where T : Object
        {
            //加载主包
            LoadMainAB();
            ...
            for (var i = 0; i < strs.Length; i++)
                ...
                if (!abDic.ContainsKey(strs[i]))
                {
                    ...
                    else
                    {
                        ...
                        var req = AssetBundle.LoadFromFileAsync(PathUrl + strs[i]);
                        yield return req;
            ...
            if (!abDic.ContainsKey(abName))
            {
                ...
                else
                {
                    ...
                    var req = AssetBundle.LoadFromFileAsync(PathUrl + abName);
                    yield return req;
                    //异步加载结束后 再替换之前的null  这时 不为null 就证明加载结束了
            ...
            if (isSync)
            {
                ...
                var res = abDic[abName].LoadAsset<T>(resName);
                callBack(res);
            }
            //异步加载包中资源
            else
            ...
            bool isSync)
        {
            ...
            if (isSync)
            {
                var res = abDic[abName].LoadAsset(resName, type);
                callBack(res);
            }
            else
            {
