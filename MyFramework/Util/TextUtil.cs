...
namespace MyFramework.Util
{
    ...
    public class TextUtil
    {
        ...
        /// <summary>
        ///     将多组键值对拼接为多行字符串（每行 "键: 值"）。
        ///     args 重载，手动收集键值。
        /// </summary>
        public static string ConcatParams(params object[] args)
        {
            return ConcatParamsCore("\n", ": ", args);
        }

        /// <summary>
        ///     将多组键值对拼接为单行字符串（每对 "键 值" 用 separator 连接）。
        /// </summary>
        public static string ConcatParamsInline(string separator, params object[] args)
        {
            return ConcatParamsCore(separator, " ", args);
        }

        /// <summary>
        ///     核心拼接：每两个元素一对（key, value），对间用 pairSeparator 连接，键值间用 kvSeparator 连接。
        /// </summary>
        private static string ConcatParamsCore(string pairSeparator, string kvSeparator, params object[] args)
        {
            ...
            for (var i = 0; i < args.Length; i += 2)
            {
                ...
                if (i > 0)
                    resultStr.Append(pairSeparator);

                resultStr.Append(key);
                resultStr.Append(kvSeparator);
        ...
        public static string[] SplitStr(string str, int type = 1)
        {
            ...
            if (type == 5) return newStr.Split(' ');

            if (type == 6) return newStr.Split('|');
        ...
        {
            //如果大于1亿 那么就显示 n亿n千万
            if (num >= 100000000) return BigDataChange(num, 100000000, "亿", "千万");
            //如果大于1万 那么就显示 n万n千
            if (num >= 10000) return BigDataChange(num, 10000, "万", "千");
            //都不满足 就直接显示数值本身
            return num.ToString();
        }
