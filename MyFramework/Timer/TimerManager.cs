...
public class TimeManager : SingletonAutoMono<TimeManager>
{
    ...
    private readonly List<TimerItem> addList = new();
    ...
    private readonly List<TimerItem> realAddList = new();
    ...
    ///     用于记录当前将要创建的唯一ID的
    /// </summary>
    private static int TIMER_KEY = 0;
    ...
    private readonly WaitForSeconds waitForSeconds = new(intervalTime);

    //为了避免内存的浪费 每次while都会生成 
    //我们直接将其声明为成员变量
    private readonly WaitForSecondsRealtime waitForSecondsRealtime = new(intervalTime);
    ...
    protected override void OnStart()
    {
        ...
        // 确保TimerItem的数据池已创建
        if (!PoolManager.Instance.HasDataPool<TimerItem>()) PoolManager.Instance.CreateDataPool<TimerItem>();
    ...
    private IEnumerator StartTiming(bool isRealTime, Dictionary<int, TimerItem> timerDic)
{
    while (true)
    {
        ...
        for (var i = list.Count - 1; i >= 0; i--)
        {
            ...
            // 关键修复：检查 Key 是否已存在
            if (timerDic.ContainsKey(item.keyID))
            {
                Debug.LogWarning($"[TimerManager] Duplicate key detected: {item.keyID}. " +
                               $"Timer will be replaced. Stack trace: {Environment.StackTrace}");
                
                // 获取旧的计时器并回收
                var oldTimer = timerDic[item.keyID];
                timerDic.Remove(item.keyID);
                PoolManager.Instance.ReturnData(oldTimer);
            }
        ...
        // 收集需要执行的回调
        var callbacksToExecute = new List<(UnityAction callback, int timerId, bool isOverCallback)>();
        ...
        foreach (var item in timerDic.Values)
        {
            ...
            try
            {
                ...
                if (item.callBack != null)
                {
                    ...
                    {
                        // 收集回调而不是直接执行
                        callbacksToExecute.Add((item.callBack, item.keyID, false));
                        item.intervalTime = item.maxIntervalTime;
                    }
                }

                // 总的时间更新
                if (!item.forever) item.allTime -= (int)(intervalTime * 1000);
                ...
                if (item.allTime <= 0)
                {
                    ...
                    {
                        // 收集结束回调
                        callbacksToExecute.Add((item.overCallBack, item.keyID, true));
                    }
                    delList.Add(item);
                }
            }
            ...
            {
                Debug.LogError($"[TimerManager] Unexpected error processing timer {item.keyID}: {e}");
                item.isRuning = false;
            }
        }

        // 遍历完成后再执行回调
        foreach (var (callback, timerId, isOverCallback) in callbacksToExecute)
        {
            try
            {
                callback?.Invoke();
            ...
            {
                Debug.LogError($"[TimerManager] Callback error for timer {timerId} (Over: {isOverCallback}): {e}");
            }
        }
    ...
    public int CreateTimer(bool isRealTime, int allTime, UnityAction overCallBack, int interval = 0,
        UnityAction callBack = null, bool forever = false)
    {
        //构建唯一ID
        var keyID = ++TIMER_KEY;
        //从缓存池取出对应的计时器
        var timerItem = PoolManager.Instance.GetData<TimerItem>();
        //初始化数据
    ...
    /// <returns>返回唯一ID 用于外部控制对应计时器</returns>
    /// <returns></returns>
    public int CreateTimer(bool isRealTime, float allTime, UnityAction overCallBack, float interval = 0f,
        UnityAction callBack = null, bool forever = false)
    {
        //构建唯一ID
        var keyID = ++TIMER_KEY;
    ...
    public void ResetTimer(int keyID)
    {
        ...
        else if (realTimerDic.ContainsKey(keyID)) realTimerDic[keyID].ResetTimer();
    }
    ...
    public void StartTimer(int keyID)
    {
        ...
        else if (realTimerDic.ContainsKey(keyID)) realTimerDic[keyID].isRuning = true;
    }

    public void TriggerHitStop(int duration = 100, float timeScale = 0f)
    {
        // 避免重复创建
        if (hitStopTimerId != -1) Instance.RemoveTimer(hitStopTimerId);

        // 设置时间缩放
        Time.timeScale = timeScale;

        // 创建不受时间缩放影响的计时器（因为timeScale可能为0）
        hitStopTimerId = Instance.CreateTimer(
    ...
    {
        // 先停止协程，防止在清空过程中有新的计时器被添加
        MonoManager.Instance.StopCoroutine(timer);
        MonoManager.Instance.StopCoroutine(realTimer);
    
        // 收集所有需要清理的计时器
        var allTimers = new List<TimerItem>();
